...
# Copier le script dans un fichier, par exemple recup_info_utilisateur.sh.
# Donner les droits d'exécution avec la commande : chmod +x recup_info_utilisateur.sh.
...
DIR_PATH="$PWD/resultats_scripts_CM"
mkdir -p "$DIR_PATH"
...
FILE_PATH="$PWD/resultats_scripts_CM/utilisateur_info_$(date +%Y%m%d_%H%M%S).txt"
